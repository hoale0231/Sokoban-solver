...
class MatrixState:
    ...
    def move(self, direction: Position):
        ...
        self.map[next.row][next.col] = POG if self.map[next.row][next.col] == GOAL else PLAYER
        self.map[self.player.row][self.player.col] = GOAL if self.map[self.player.row][self.player.col] == POG else FLOOR

        self.map[next.row][next.col] = POD if self.map[next.row][next.col] == DEADLOCK else PLAYER
        self.map[self.player.row][self.player.col] = DEADLOCK if self.map[self.player.row][self.player.col] == POD else FLOOR
